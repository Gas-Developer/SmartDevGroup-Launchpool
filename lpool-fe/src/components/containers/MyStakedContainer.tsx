...
export default function MyStakedContainer(props: any) {
	...
	const { allLaunchpoolReferenceGContext, ipfsDataGContext } =
		useGlobalContext();

	const PolygonScanBaseURL = process.env.NEXT_PUBLIC_POLYGONSCAN_API_URL;
	const PolygonScanActionURL =
		"?module=account&action=txlist&&startblock=0&endblock=99999999&page=1&offset=50&sort=desc";
	const PolygonScanAPIKEYURL =
		"&apikey=" + process.env.NEXT_PUBLIC_POLYGONSCAN_API_KEY;
	//const PolygonScanAddress = "&address=0xf40CcC851DBB641a28baB94D2dAB81bb549ffD8a";
	const PolygonScanAddress =
		"&address=0x44aae84107ffa4f3a1c0382648d726b56cb60020";
	...
	useEffect(() => {
		...
		const PolygonScanAPICallURL =
			PolygonScanBaseURL +
			PolygonScanActionURL +
			PolygonScanAPIKEYURL +
			PolygonScanAddress;
			...
			.then((res) => {
				...
				if (res !== undefined) {
					...
					res.data.result.forEach(function (
						...
						if (
							item.functionName ==
								"deployClone(address _implementationContract,address _token,uint256 _startLP,uint256 _endLP,string storageURI)" ||
							item.functionName ==
								"depositTokenToDistribute(uint256 _amount)" ||
	...
	function getMyStaked() {
		...
		const aggregatedMyStaked: AggregatedTransaction[] = Array.from(
		...
		const allLaunchpoolsWithPhase = getAllLaunchpoolsWithIPFSDataAndPhase(
			allLaunchpoolReferenceGContext,
			ipfsDataGContext
		);
		...
		aggregatedMyStaked.forEach((stakedItem) => {
			const cleanedStakedTo = stakedItem.to
				.replace(/\s+/g, "")
				.toLowerCase(); // Rimuove tutti gli spazi
			const matchingLaunchpool = allLaunchpoolsWithPhase.find(
				(lp) => lp.launchpoolAddress.toLowerCase() === cleanedStakedTo
			);

			if (matchingLaunchpool) {
				const index =
					allLaunchpoolsWithPhase.indexOf(matchingLaunchpool);
	...
	function getAllLaunchpoolsWithIPFSDataAndPhase(
	...
	return (
		<>
			<TrasparentContainer className={" w-2/3"}>
				<ul className=" text-slate-200 text-sm">
					{myStaked?.map((stakeTX: MyLPs, index: number) => (
						<li key={index} id={`staked${index}`}>
							<DefaultContainer className=" defaultContainer align-middle m-2 grid grid-cols-4">
								<div className="text-xs text-slate-500 p-4 align-middle col-span-1">
									<Image
										className=""
								...
								<div className="text-xs text-slate-500 align-middle col-span-2 m-auto">
									<InfoLabel
										name={"LPNameValue"}
										value={stakeTX.name}
										className={""}
								...
								<div className="text-xs text-slate-500 text-right align-middle p-3 col-span-1 mb-auto mt-auto">
									Phase: {stakeTX.phase}
								</div>
							</DefaultContainer>
						</li>
