...
import { stringify } from "querystring";
	...
	const LaunchpoolFactory = await ethers.getContractFactory("LaunchpoolFactory_EB");										// Cotruisco la factory 
	...
	// Esegui una query per ottenere l'ultimo evento emesso
	const filter = await launchpoolFactory.filters.ProxyCreated();
	const events = await launchpoolFactory.queryFilter(filter);

	if (events == null || events.length == 0 || events[0].args == null || events[0].args[0].length == 0) {
		throw new Error("No events found");
	}

	const newLaunchpoolAddress =  events[0].args[0];
	console.log("address Proxy:", newLaunchpoolAddress);
